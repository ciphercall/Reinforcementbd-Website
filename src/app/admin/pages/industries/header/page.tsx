...
export default function IndustriesHeaderEditor() {
  const router = useRouter()
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)
  const [content, setContent] = useState<HeaderContent>({
    title: 'Industries We Serve',
    subtitle: 'Delivering specialized solutions across diverse sectors with deep industry expertise and proven track record.',
  ...
  const fetchContent = async () => {
    try {
      const res = await fetch('/api/page-content?page=industries&section=header')
      if (res.ok) {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <Button
              variant="outline"
              size="sm"
              onClick={() => router.push('/admin/pages/industries')}
            >
            ...
            <div>
              ...
              <p className="text-gray-600">Edit the Industries page hero section</p>
            </div>
          </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="bg-gradient-to-r from-purple-900 to-purple-700 rounded-lg p-8 text-white">
              ...
              <p className="text-purple-100 max-w-2xl">{content.subtitle}</p>
            </div>
          </CardContent>
        </Card>
      </div>
