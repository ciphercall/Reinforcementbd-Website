...
export default function ServicesHeaderEditor() {
  ...
  const [content, setContent] = useState<HeaderContent>({
    title: 'Our Services',
    subtitle: 'Comprehensive business solutions tailored to your needs. From IT services to human resources, we have you covered.',
  ...
  const fetchContent = async () => {
    try {
      const res = await fetch('/api/page-content?page=services&section=header')
      if (res.ok) {
    ...
    <AdminSidebar>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <Button
              variant="outline"
              size="sm"
              onClick={() => router.push('/admin/pages/services')}
            >
            ...
            <div>
              ...
              <p className="text-gray-600">Edit the Services page hero section</p>
            </div>
          </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="bg-gradient-to-r from-green-900 to-green-700 rounded-lg p-8 text-white">
              <p className="text-sm opacity-80 mb-2">Home / {content.breadcrumbLabel}</p>
              <h2 className="text-3xl font-bold mb-4">{content.title}</h2>
              <p className="text-green-100 max-w-2xl">{content.subtitle}</p>
            </div>
          </CardContent>
        </Card>
      </div>
