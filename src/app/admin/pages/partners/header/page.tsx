...
export default function PartnersHeaderEditor() {
  ...
  const fetchContent = async () => {
    try {
      const res = await fetch('/api/page-content?page=partners&section=header')
      if (res.ok) {
    ...
    <AdminSidebar>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <Button
              variant="outline"
              size="sm"
              onClick={() => router.push('/admin/pages/partners')}
            >
            ...
            <div>
              ...
              <p className="text-gray-600">Edit the Partners page hero section</p>
            </div>
          </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="bg-gradient-to-r from-orange-900 to-orange-700 rounded-lg p-8 text-white">
              <p className="text-sm opacity-80 mb-2">Home / {content.breadcrumbLabel}</p>
              <h2 className="text-3xl font-bold mb-4">{content.title}</h2>
              <p className="text-orange-100 max-w-2xl">{content.subtitle}</p>
            </div>
          </CardContent>
        </Card>
      </div>
